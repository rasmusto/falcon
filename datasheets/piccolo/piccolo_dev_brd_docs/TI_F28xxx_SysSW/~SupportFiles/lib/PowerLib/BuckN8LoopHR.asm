...
;  Date	     | Description
;----------------------------------------------------------------------------------
;  29 Aug 07 | Release 0.1  		Uses Pre/Post calc and HR option (DAF)
;----------------------------------------------------------------------------------
		;.include "Eperipheral_registers.h"

;=================================================================================
;	BuckN8LoopHR_DRV  macro module - 1~8 channel HiRes Buck loops
;=================================================================================
...
;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  I N I T - START  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
;===============================
BuckN8LoopHR_DRV_INIT	.macro n
;===============================
; Variable Declarations
; Note: "Buck1Loop" - loops 1 to 4, is on 1st DP boundary
_Buck_Ref			.usect "Buck1Loop",2,1,1	; Input Terminal ptr to "Array Bus"
_Buck_Out			.usect "Buck1Loop",2 		; Output Terminal ptr to "Array Bus" (size = 2)
_Buck_Fdbk			.usect "Buck1Loop",2		; Output Terminal ptr to "Array Bus" (size = 1)
...
_SFBUCK1n			.usect "Buck1Loop",1 		; ScaleFactor for all Channels
...
;Publish Terminal Pointers for access from the C environment (optional)
					...
					.def 	_SFBUCK1n			; Variable
					.def 	_SFBUCK2n			; Variable
...
; Used as starting value for HiRes ScaleFactor
	MOV		@_SFBUCK1n, #(70*256)		; Initial value for 100MHz case

	MOVW	DP,#buck_period2
	MOV		@buck_period2, ACC			; buck_period2=(TBPRDn x 2)
; Used as starting value for HiRes ScaleFactor
	MOV		@_SFBUCK2n, #(70*256)		; Initial value for 100MHz case
					.endm

;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  R U N - START  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
;========================
BuckN8LoopHR_DRV	.macro n
;========================
;================================  L O O P - 1  =======================================
; Set up address pointers: (30 / 28 cycles)
Loop1:
	;SETC    SXM,OVM
	...
	; For debug or to monitor Uout & Vfdbk - comment to save cycle
	MOVL	XAR4, @_Buck_Out			; Net "Bus" pointer to Out in array (XAR4)
	MOVL	XAR5, @_Buck_Fdbk			; Net "Bus" pointer to FeedBack (XAR5)
	MOV 	ACC, *XAR1<<3				; Normalize to PerUnit
	MOV 	*XAR5++	,AL					; Q15 format, Unipolar input
...
;================================  L O O P - 2  =======================================
	.if(n>1);Loop2
Loop2:
; Set up address pointers: (32 cycles)
...
; Output to EPWM2A - HIGH RES (7 cyc)
	MOV 	T,@AH						; Need only use upper 16 bits of Duty
	MPYU 	ACC,T,@buck_period1
	MOV		T,@_SFBUCK1n				; T = ScaleFactor
	MPYU	P,T,@AL						; P = T * AL
...
; Output to EPWM3A - HIGH RES (7 cyc)
	MOV 	T,@AH						; Need only use upper 16 bits of Duty
	MPYU 	ACC,T,@buck_period1
	MOV		T,@_SFBUCK1n				; T = ScaleFactor
	MPYU	P,T,@AL						; P = T * AL
...
; Output to EPWM4A - HIGH RES (7 cyc)
	MOV 	T,@AH						; Need only use upper 16 bits of Duty
	MPYU 	ACC,T,@buck_period1
	MOV		T,@_SFBUCK1n				; T = ScaleFactor
	MPYU	P,T,@AL						; P = T * AL
...
; Output to EPWM5A - HIGH RES (7 cyc)
	MOV 	T,@AH						; Need only use upper 16 bits of Duty
	MPYU 	ACC,T,@buck_period2
	MOV		T,@_SFBUCK2n				; T = ScaleFactor
	MPYU	P,T,@AL						; P = T * AL
	MOVH	@AL,P
	ADD		ACC, #0x17F					; Optimize by Round up
	MOVL	*XAR2,ACC 					; CMPA:CMPAHR(31:8) = ACC
...
; Output to EPWM6A - HIGH RES (7 cyc)
	MOV 	T,@AH						; Need only use upper 16 bits of Duty
	MPYU 	ACC,T,@buck_period2
	MOV		T,@_SFBUCK2n				; T = ScaleFactor
	MPYU	P,T,@AL						; P = T * AL
...
; Output to EPWM7A - HIGH RES (7 cyc)
	MOV 	T,@AH						; Need only use upper 16 bits of Duty
	MPYU 	ACC,T,@buck_period2
	MOV		T,@_SFBUCK2n				; T = ScaleFactor
	MPYU	P,T,@AL						; P = T * AL
...
; Output to EPWM8A - HIGH RES (7 cyc)
	MOV 	T,@AH						; Need only use upper 16 bits of Duty
	MPYU 	ACC,T,@buck_period2
	MOV		T,@_SFBUCK2n				; T = ScaleFactor
	MPYU	P,T,@AL						; P = T * AL
